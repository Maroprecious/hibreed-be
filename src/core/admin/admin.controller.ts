...
import { AboutUsDto, BodyDataDto, HowItWorkssDto, TestimonialArrsDto, UpdateAdminDto, UpdateHeroDto } from "./dto/admin.dto";
...
export class AdminController {
    ...
    @Get("about-us")
    async aboutUs() {
        return await this.adminService.getAboutUs()
    }
    ...
    @HttpCode(HttpStatus.CREATED)
    @Post("about-us/hero-section")
    @UseInterceptors(FileInterceptor('hero_image'))
    async updateAboutUsHero(
        @UploadedFile(
            new ParseFilePipeBuilder()
                .addFileTypeValidator({
                    fileType: /jpeg|jpg|png/,
        ...
        @Body() body: AboutUsDto) {
        return await this.adminService.updateAboutUsHero(body, hero_image)
    }
    ...
    @Post("about-us/body")
    @UseInterceptors(AnyFilesInterceptor())
    async updateAboutUsBody(
        @UploadedFiles() images: Express.Multer.File[],
        @Body() payload: BodyDataDto) {
        return await this.adminService.updateAboutUsBody(payload, images)
    }
}